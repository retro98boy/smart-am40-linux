...
static void vpd_extcon_irq_work(struct work_struct *work)
{
	struct vpd_extcon *vpd =
		container_of(work, struct vpd_extcon, irq_work.work);
	...
	if (det) {
		...
		dp_connected = (vpd->data_role == DR_DISPLAY_PORT) ? true :
								     false;
	}
	...
	extcon_set_property(vpd->extcon, EXTCON_USB, EXTCON_PROP_USB_SS,
			    property);
	extcon_set_property(vpd->extcon, EXTCON_USB_HOST, EXTCON_PROP_USB_SS,
			    property);
	extcon_set_property(vpd->extcon, EXTCON_DISP_DP, EXTCON_PROP_USB_SS,
			    property);
...
static int vpd_extcon_parse_dts(struct vpd_extcon *vpd)
{
	...
	ret = devm_request_threaded_irq(
		dev, vpd->irq, NULL, vpd_extcon_irq_handler,
		IRQF_TRIGGER_FALLING | IRQF_TRIGGER_RISING | IRQF_ONESHOT, NULL,
		vpd);
	if (ret)
...
static SIMPLE_DEV_PM_OPS(vpd_extcon_pm_ops, vpd_extcon_suspend,
			 vpd_extcon_resume);

static const struct of_device_id vpd_extcon_dt_match[] = {
	{
		.compatible = "linux,extcon-usbc-virtual-pd2",
	},
	{ /* sentinel */ }
};
